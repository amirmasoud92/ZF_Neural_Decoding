# PeakRate ---- Compare Performance LFP vs MUAe
...
setwd(Load_Path)
fileIn <- 'Fig4_C_PeakRate_Session.csv'
SaveName <- 'PeakRate_Depths_LFPvsMUAe.csv'
...
df3A.PeakRate.long <- pivot_longer(df3A.PeakRate, cols = c("Perf_LFP", "Perf_MUAe"), names_to = 'Signal', values_to = 'Performance')

# Fit null and effect models for performance comparison
null_model <- lmer(Performance ~ Depth + (1 | Session), data = df3A.PeakRate.long, REML = FALSE)
effect_model <- lmer(Performance ~ Signal * Depth + (1 | Session), data = df3A.PeakRate.long, REML = FALSE)
