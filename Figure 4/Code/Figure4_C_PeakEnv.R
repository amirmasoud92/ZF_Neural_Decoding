...
setwd(Load_Path)
fileIn <- 'Fig4_C_PeakEnv.csv'
SaveName <- 'Fig4_PeakEnv_Depths.csv'

df3A.PeakEnv <- read.csv(fileIn)

# Section 2: Data Preprocessing
# Convert relevant columns to factors
df3A.PeakEnv <- within(df3A.PeakEnv, {
...
null_model_LFP <- lmer(Perf_LFP ~ 1 + (1 | Birds_Name), data = df3A.PeakEnv, REML = FALSE)
effect_model_LFP <- lmer(Perf_LFP ~ Depth + (1 | Birds_Name), data = df3A.PeakEnv, REML = FALSE)
...
doc <- body_add_par(doc, paste0("**LFP PeakEnv: ", formatted_result_LFP, "**"), style = "Normal")
...
null_model_MUAe <- lmer(Perf_MUAe ~ 1 + (1 | Birds_Name), data = df3A.PeakEnv, REML = FALSE)
effect_model_MUAe <- lmer(Perf_MUAe ~ Depth + (1 | Birds_Name), data = df3A.PeakEnv, REML = FALSE)
...
doc <- body_add_par(doc, paste0("**MUAe PeakEnv Detection: ", formatted_result_MUAe, "**"), style = "Normal")
