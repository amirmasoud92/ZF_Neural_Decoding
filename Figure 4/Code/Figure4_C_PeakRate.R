...
setwd(Load_Path)
fileIn <- 'Fig4_C_PeakRate.csv'
SaveName <- 'Fig4_PeakRate_Depths.csv'

df3A.PeakRate <- read.csv(fileIn)

# Section 2: Data Preprocessing
# Convert relevant columns to factors
df3A.PeakRate <- within(df3A.PeakRate, {
...
null_model_LFP <- lmer(Perf_LFP ~ 1 + (1 | Birds_Name), data = df3A.PeakRate, REML = FALSE)
effect_model_LFP <- lmer(Perf_LFP ~ Depth + (1 | Birds_Name), data = df3A.PeakRate, REML = FALSE)
...
doc <- body_add_par(doc, paste0("**LFP PeakRate: ", formatted_result_LFP, "**"), style = "Normal")
...
null_model_MUAe <- lmer(Perf_MUAe ~ 1 + (1 | Birds_Name), data = df3A.PeakRate, REML = FALSE)
effect_model_MUAe <- lmer(Perf_MUAe ~ Depth + (1 | Birds_Name), data = df3A.PeakRate, REML = FALSE)
...
doc <- body_add_par(doc, paste0("**MUAe PeakRate Detection: ", formatted_result_MUAe, "**"), style = "Normal")
