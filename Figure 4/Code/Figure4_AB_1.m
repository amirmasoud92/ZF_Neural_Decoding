...
SizeScale = importdata(fullfile(paths.data,'SizeCh_LFP_Event_Final.mat'));
SizeScale = abs(SizeScale);


%% LFP Performance
LFP_EventPerf = importdata(fullfile(paths.data,'LFP_Event_Detect_Results_Table_New3.mat'));

%% Perf Sessions
for i = 1:51

    LFP_S_Perf(i) = mean(LFP_EventPerf.Performance(find(LFP_EventPerf.Session_Number==i)));
...
for i = 1:size(Channel_Depth_Perf_MUAe,1)
    for j = 1:size(Channel_Depth_Perf_MUAe,2)
        if ~isnan(Channel_Depth_Perf_MUAe(i,j))
            ...
            scatter3(Loc_Channel_all(i,1),Loc_Channel_all(i,2),Depth_Uniq(j),20*SizeCh,Channel_Depth_Perf_MUAe(i,j),'filled','MarkerEdgeColor','k');
...
title('Event Detection (LFP)','FontSize',28,'FontWeight','bold','FontName','Arial')
...
videoFileName = 'LFP_EventDetection.mp4';
...
SizeScale = importdata(fullfile(paths.data,'SizeCh_MUAe_Event_Final.mat'));

SizeScale =  abs(SizeScale);

%% MUAe Performance

MUAe_EventPerf = importdata(fullfile(paths.data,'MUAe_Event_Detect_Results_Table_New3.mat'));

%% Perf Sessions

for i = 1:51

    MUAe_S_Perf(i) = mean(MUAe_EventPerf.Performance(find(MUAe_EventPerf.Session_Number==i)));
...
title('Event Detection (MUAe)','FontSize',28,'FontWeight','bold','FontName','Arial')
...
videoFileName = 'MUAe_EventDetection.mp4';
v = VideoWriter(videoFileName, 'MPEG-4');
v.FrameRate = 10; 
open(v);
