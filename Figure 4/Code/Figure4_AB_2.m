...
SizeScale = importdata(fullfile(paths.data,'SizeCh_LFP_Envelope_Final.mat'));


%% LFP Performance

LFP_EnvPerf = importdata(fullfile(paths.data,'LFP_NarrowBand_Decoding_Results_Table_New.mat'));
...
for i = 1:51
    ...
    LFP_S_Perf(i) = mean(LFP_EnvPerf.R2_Performance(find(LFP_EnvPerf.Session_Number==i)));
...
title('Envelope Decoding (LFP)','FontSize',28,'FontWeight','bold','FontName','Arial')
...
videoFileName = 'LFP_EnvelopeDecoding.mp4';
...
h = colorbar;
set(get(h,'label'),'string','R²','Rotation',270);
...
SizeScale = importdata(fullfile(paths.data,'SizeCh_MUAe_Envelope_Final.mat'));

%% MUAe Performance

MUAe_EnvelopePerf = importdata(fullfile(paths.data,'MUAe_NarrowBand_Decoding_Results_Table_New.mat'));

%% Perf Sessions
for i = 1:51
    MUAe_S_Perf(i) = mean(MUAe_EnvelopePerf.R2_Performance(find(MUAe_EnvelopePerf.Session_Number==i)));
end 
%%
Depth_Uniq = depth_all;
...
title('Envelope Decoding (MUAe)','FontSize',28,'FontWeight','bold','FontName','Arial')
...
videoFileName = 'MUAe_EnvelopeDecoding.mp4';
v = VideoWriter(videoFileName, 'MPEG-4');
v.FrameRate = 10;
open(v);
