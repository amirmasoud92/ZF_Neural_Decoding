# PeakEnv ---- Compare Performance LFP vs MUAe
...
setwd(Load_Path)
fileIn <- 'Fig4_C_PeakEnv_Session.csv'
SaveName <- 'PeakEnv_Depths_LFPvsMUAe.csv'
...
df3A.PeakEnv.long <- pivot_longer(df3A.PeakEnv, cols = c("Perf_LFP", "Perf_MUAe"), names_to = 'Signal', values_to = 'Performance')

# Fit null and effect models for performance comparison
null_model <- lmer(Performance ~ Depth + (1 | Session), data = df3A.PeakEnv.long, REML = FALSE)
effect_model <- lmer(Performance ~ Signal * Depth + (1 | Session), data = df3A.PeakEnv.long, REML = FALSE)
