######################################
# Figure S8 - PeakEnv  - Decoding performance of temporal features for each song using both LFP and MUAe.
######################################
...
save_name <- 'PeakEnv_Fusion.csv'

df_PeakEnv <- read.csv(file_in)

# Data Preprocessing
# Convert relevant columns to factors
df_PeakEnv <- transform(df_PeakEnv, 
...
doc <- decode_analysis(
  null_model_formula = (MeanFusion_PeakEnv - MeanLFP_PeakEnv) ~ -1 + (1 | Birds_Name),
  effect_model_formula = (MeanFusion_PeakEnv - MeanLFP_PeakEnv) ~ 1 + (1 | Birds_Name),
  data = df_PeakEnv,
  label = "LFP PeakEnv",
  save_path = save_path,
  save_name = save_name,
...
doc <- decode_analysis(
  null_model_formula = (MeanFusion_PeakEnv - MeanMUAe_PeakEnv) ~ -1 + (1 | Birds_Name),
  effect_model_formula = (MeanFusion_PeakEnv - MeanMUAe_PeakEnv) ~ 1 + (1 | Birds_Name),
  data = df_PeakEnv,
  label = "MUAe PeakEnv Detection",
  save_path = save_path,
  save_name = save_name,
  doc = doc
)
