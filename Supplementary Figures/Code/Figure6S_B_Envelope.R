######################################
## Figure S6 - LFP - Envelope Detection - Comparison of decoding performance of temporal song features based on playback order within each session.
######################################
...
setwd(Load_Path)
fileIn <- 'FigS6_Envelope.csv'
SaveName <- 'Envelope_OrderParts.csv'
...
null_model <- lmer(LFP_Perf ~ Depth + (1 | Birds_Name), data = df3A.Envelope)
effect_model <- lmer(LFP_Perf ~ Parts + Depth + (1 | Birds_Name), data = df3A.Envelope)
...
print(paste0('**LFP Envelope: ', formatted_result_acc, '**'))

# Section 5: Model Fitting and Analysis for MUAe Performance
null_model_muae <- lmer(MUAe_Perf ~ Depth + (1 | Birds_Name), data = df3A.Envelope)
effect_model_muae <- lmer(MUAe_Perf ~ Parts + Depth + (1 | Birds_Name), data = df3A.Envelope)
...
print(paste0('**MUAe Envelope Detection: ', formatted_result_muae, '**'))
