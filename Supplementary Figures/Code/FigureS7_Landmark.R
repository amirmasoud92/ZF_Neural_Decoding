######################################
## Figure S7 - Landmark - Decoding performance of temporal features for each song using both LFP and MUAe.
######################################
...
setwd(Load_Path)
fileIn <- 'FigS7_Landmark.csv'
SaveName <- 'Landmark_SongN.csv'
...
null_model_LFP <- lmer(Perf_LFP ~ Depth + (1 | Birds_Name), data = df3A.Landmark, REML = FALSE)
effect_model_LFP <- lmer(Perf_LFP ~ SongN + Depth + (1 | Birds_Name), data = df3A.Landmark, REML = FALSE)
...
doc <- body_add_par(doc, paste0("**LFP Landmark: ", formatted_result_LFP, "**"), style = "Normal")
...
null_model_MUAe <- lmer(Perf_MUAe ~ Depth + (1 | Birds_Name), data = df3A.Landmark, REML = FALSE)
effect_model_MUAe <- lmer(Perf_MUAe ~ SongN + Depth + (1 | Birds_Name), data = df3A.Landmark, REML = FALSE)
...
doc <- body_add_par(doc, paste0("**MUAe Landmark Detection: ", formatted_result_MUAe, "**"), style = "Normal")
