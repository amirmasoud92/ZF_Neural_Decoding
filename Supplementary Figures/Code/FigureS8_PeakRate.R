######################################
# Figure S8 - PeakRate  - Decoding performance of temporal features for each song using both LFP and MUAe.
######################################
...
setwd(load_path)
file_in <- 'FigS8_Landmark.csv'
save_name <- 'PeakRate_Fusion.csv'

df_PeakRate <- read.csv(file_in)

# Data Preprocessing
# Convert relevant columns to factors
df_PeakRate <- transform(df_PeakRate, 
...
doc <- decode_analysis(
  null_model_formula = (MeanFusion_PeakRate - MeanLFP_PeakRate) ~ -1 + (1 | Birds_Name),
  effect_model_formula = (MeanFusion_PeakRate - MeanLFP_PeakRate) ~ 1 + (1 | Birds_Name),
  data = df_PeakRate,
  label = "LFP PeakRate",
  save_path = save_path,
  save_name = save_name,
...
doc <- decode_analysis(
  null_model_formula = (MeanFusion_PeakRate - MeanMUAe_PeakRate) ~ -1 + (1 | Birds_Name),
  effect_model_formula = (MeanFusion_PeakRate - MeanMUAe_PeakRate) ~ 1 + (1 | Birds_Name),
  data = df_PeakRate,
  label = "MUAe PeakRate Detection",
  save_path = save_path,
  save_name = save_name,
  doc = doc
)
