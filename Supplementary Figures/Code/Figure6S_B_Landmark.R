######################################
## Figure S6 - LFP - Landmark Detection - Comparison of decoding performance of temporal song features based on playback order within each session.
######################################
...
setwd(Load_Path)
fileIn <- 'FigS6_Landmark.csv'
SaveName <- 'Landmark_OrderParts.csv'

df3A.Landmark <- read.csv(fileIn)

# Section 2: Data Preprocessing
# Convert relevant columns to factors
df3A.Landmark <- within(df3A.Landmark, {
...
# Summarize the data
summary(df3A.Landmark)

# Section 3: Model Fitting and Analysis for LFP Performance
# Fit null and effect models
null_model <- lmer(LFP_Perf ~ Depth + (1 | Birds_Name), data = df3A.Landmark)
effect_model <- lmer(LFP_Perf ~ Parts + Depth + (1 | Birds_Name), data = df3A.Landmark)
...
formatted_result_acc <- sprintf('χ^2 (%d) = %.2f, p = %.8f; Δr = %.2f ± %.3f SE', df, chi_square, p_value, delta_r, se)
print(paste0('**LFP Landmark: ', formatted_result_acc, '**'))

# Section 5: Model Fitting and Analysis for MUAe Performance
null_model_muae <- lmer(MUAe_Perf ~ Depth + (1 | Birds_Name), data = df3A.Landmark)
effect_model_muae <- lmer(MUAe_Perf ~ Parts + Depth + (1 | Birds_Name), data = df3A.Landmark)
...
formatted_result_muae <- sprintf('χ^2 (%d) = %.2f, p = %.8f; Δr = %.2f ± %.3f SE', df_muae, chi_square_muae, p_value_muae, delta_r_muae, se_muae)
print(paste0('**MUAe Landmark Detection: ', formatted_result_muae, '**'))
