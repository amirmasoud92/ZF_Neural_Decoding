...
# Load the CSV data
data <- read.csv(file.path(load_path, "Fig6_ITPCz_Event.csv"))
...
cat(output_text, file = paste0(save_path, "Fig6_Event__ITPCz_Linear_Mixed_Model_Summary.doc"))

write.csv(as.data.frame(coefs), file = paste0(save_path, "Fig6_Event__ITPCz_Model_Coefficients.csv"))
...
cat(cor_summary_text, file = paste0(save_path, "Fig6_ITPCz_PerfEvent_Correlation.doc"))
...
CairoSVG(
  file = paste0(save_path, "Fig6_Event__ITPCz_Performance_vs_ITPCz.svg"), 
  width = 10, 
  height = 10, 
  pointsize = 12
)
