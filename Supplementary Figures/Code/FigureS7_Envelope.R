######################################
## Figure S7 - Envelope - Decoding performance of temporal features for each song using both LFP and MUAe.
######################################
...
setwd(Load_Path)
fileIn <- 'FigS7_Envelope.csv'
SaveName <- 'Envelope_SongN.csv'
...
null_model_LFP <- lmer(Perf_LFP ~ Depth + (1 | Birds_Name), data = df3A.Envelope, REML = FALSE)
effect_model_LFP <- lmer(Perf_LFP ~ SongN + Depth + (1 | Birds_Name), data = df3A.Envelope, REML = FALSE)
...
null_model_MUAe <- lmer(Perf_MUAe ~ Depth + (1 | Birds_Name), data = df3A.Envelope, REML = FALSE)
effect_model_MUAe <- lmer(Perf_MUAe ~ SongN + Depth + (1 | Birds_Name), data = df3A.Envelope, REML = FALSE)
