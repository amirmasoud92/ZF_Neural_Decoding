######################################
## Figure 3 - Part B - LandMarks Detection - LFP vs. MUAe
######################################
...
setwd(Load_Path)
fileIn <- 'Fig3_B_3.csv'
SaveName <- 'LandMarksSessionLFP-MUA.csv'
...
null_model <- lmer('(Acc_MUAe_PeakRate - Acc_LFP_PeakRate) ~ -1 + (1 | Birds_Name)', data = df3A.event)
effect_model <- lmer('(Acc_MUAe_PeakRate - Acc_LFP_PeakRate) ~ 1 + (1 | Birds_Name)', data = df3A.event)
...
mean_diff <- mean(df3A.event$Acc_MUAe_PeakRate - df3A.event$Acc_LFP_PeakRate)
se_diff <- sd(df3A.event$Acc_MUAe_PeakRate - df3A.event$Acc_LFP_PeakRate) / sqrt(nrow(df3A.event))
...
null_model_cohen <- lmer('(Acc_MUAe_PeakEnv - Acc_LFP_PeakEnv) ~ -1 + (1 | Birds_Name)', data = df3A.event)
effect_model_cohen <- lmer('(Acc_MUAe_PeakEnv - Acc_LFP_PeakEnv) ~ 1 + (1 | Birds_Name)', data = df3A.event)
...
mean_diff_cohen <- mean(df3A.event$Acc_MUAe_PeakEnv - df3A.event$Acc_LFP_PeakEnv)
se_diff_cohen <- sd(df3A.event$Acc_MUAe_PeakEnv - df3A.event$Acc_LFP_PeakEnv) / sqrt(nrow(df3A.event))
