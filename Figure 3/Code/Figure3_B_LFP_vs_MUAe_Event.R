######################################
## Figure 3 - Part B - Event Detection - Accuracy - LFP vs. MUAe
######################################
...
setwd(Load_Path)
fileIn <- 'Fig3_B_1.csv'
SaveName <- 'EventSessionLFP-MUA.csv'
...
null_model <- lmer('(ACC_Session_MUAe - ACC_Session_LFP) ~ -1 + (1 | Birds_Name)', data = df3A.event)
effect_model <- lmer('(ACC_Session_MUAe - ACC_Session_LFP) ~ 1 + (1 | Birds_Name)', data = df3A.event)
...
mean_diff <- mean(df3A.event$ACC_Session_MUAe - df3A.event$ACC_Session_LFP)
se_diff <- sd(df3A.event$ACC_Session_MUAe - df3A.event$ACC_Session_LFP) / sqrt(nrow(df3A.event))
formatted_result_data_acc <- sprintf('From Data: Acc Difference MUAe - LFP = %.2f ± %.3f', mean_diff, se_diff)
print(formatted_result_data_acc)

# Fit null and effect models for Cohen
null_model_cohen <- lmer('(Cohen_Session_MUAe - Cohen_Session_LFP) ~ -1 + (1 | Birds_Name)', data = df3A.event)
effect_model_cohen <- lmer('(Cohen_Session_MUAe - Cohen_Session_LFP) ~ 1 + (1 | Birds_Name)', data = df3A.event)
...
mean_diff_cohen <- mean(df3A.event$Cohen_Session_MUAe - df3A.event$Cohen_Session_LFP)
se_diff_cohen <- sd(df3A.event$Cohen_Session_MUAe - df3A.event$Cohen_Session_LFP) / sqrt(nrow(df3A.event))
formatted_result_data_cohen <- sprintf('From Data: Cohen Difference MUAe - LFP = %.2f ± %.3f', mean_diff_cohen, se_diff_cohen)
