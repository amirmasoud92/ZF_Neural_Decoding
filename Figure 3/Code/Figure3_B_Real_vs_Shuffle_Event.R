######################################
## Figure 3 - Part B -Event Detection - LFP - Comparison of LFP with MUAe performance for decoding song temporal features (Sessions) 
######################################
...
setwd(Load_Path)
fileIn <- 'Fig3_B_1.csv'
SaveName <- 'EventRealvsShuffle.csv'
...
# Section 3: Model fitting and analysis for Acc
# Fit null and effect models
null_model <- lmer('(ACC_Session_LFP - ACC_Shuffle_LFP) ~ -1 + (1 | Birds_Name)', data = df3A.event)
effect_model <- lmer('(ACC_Session_LFP - ACC_Shuffle_LFP) ~ 1 + (1 | Birds_Name)', data = df3A.event)
...
# Section 4: Print formatted results for Acc
...
mean_diff <- mean(df3A.event$ACC_Session_LFP - df3A.event$ACC_Shuffle_LFP)
se_diff <- sd(df3A.event$ACC_Session_LFP - df3A.event$ACC_Shuffle_LFP) / sqrt(nrow(df3A.event))
...
# Section 5: Cohen analysis
# Fit null and effect models for Cohen
null_model_cohen <- lmer('(ACC_Session_MUAe - ACC_Shuffle_MUAe) ~ -1 + (1 | Birds_Name)', data = df3A.event)
effect_model_cohen <- lmer('(ACC_Session_MUAe - ACC_Shuffle_MUAe) ~ 1 + (1 | Birds_Name)', data = df3A.event)
...
# Section 6: Print formatted results for Cohen
...
mean_diff_cohen <- mean(df3A.event$Cohen_Session_LFP - df3A.event$Cohen_Shuffle_LFP)
se_diff_cohen <- sd(df3A.event$Cohen_Session_LFP - df3A.event$Cohen_Shuffle_LFP) / sqrt(nrow(df3A.event))
