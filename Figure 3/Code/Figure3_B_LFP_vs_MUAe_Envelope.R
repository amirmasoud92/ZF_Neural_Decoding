######################################
## Figure 3 - Part B -Envelope Detection - R/ Correlation - LFP vs. MUAe
######################################
...
setwd(Load_Path)
fileIn <- 'Fig3_B_2.csv'
SaveName <- 'EnvSessionLFP-MUA.csv'
...
null_model <- lmer('(Corr_Session_MUAe - Corr_Session_LFP) ~ -1 + (1 | Birds_Name)', data = df3A.event)
effect_model <- lmer('(Corr_Session_MUAe - Corr_Session_LFP) ~ 1 + (1 | Birds_Name)', data = df3A.event)
...
mean_diff <- mean(df3A.event$Corr_Session_MUAe - df3A.event$Corr_Session_LFP)
se_diff <- sd(df3A.event$Corr_Session_MUAe - df3A.event$Corr_Session_LFP) / sqrt(nrow(df3A.event))
formatted_result_data_acc <- sprintf('From Data: Corr Difference MUAe - LFP = %.2f ± %.3f', mean_diff, se_diff)
print(formatted_result_data_acc)

# Fit null and effect models for Cohen
null_model_cohen <- lmer('(R2_Session_MUAe - R2_Session_LFP) ~ -1 + (1 | Birds_Name)', data = df3A.event)
effect_model_cohen <- lmer('(R2_Session_MUAe - R2_Session_LFP) ~ 1 + (1 | Birds_Name)', data = df3A.event)
...
mean_diff_cohen <- mean(df3A.event$R2_Session_MUAe - df3A.event$R2_Session_LFP)
se_diff_cohen <- sd(df3A.event$R2_Session_MUAe - df3A.event$R2_Session_LFP) / sqrt(nrow(df3A.event))
formatted_result_data_cohen <- sprintf('From Data: R2 Difference MUAe - LFP = %.2f ± %.3f', mean_diff_cohen, se_diff_cohen)
